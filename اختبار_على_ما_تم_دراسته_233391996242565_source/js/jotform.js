    ...
    init: function (callback) {
        var ready = function () {
            try {
                ...
                this.sessionID = ('session' in document.get) && document.get.session.indexOf('JF-S4L') !== -1 && (document.get.session.length > 0) ? document.get.session : false;
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    setBirtDateEvents: function () {
        var birtdateEls = document.querySelectorAll('[data-type="control_birthdate"]');

        function addOption(el, mounth, year) {
            var count = new Date(year, mounth, 0).getDate();
            el.innerHTML = '';
        ...
        birtdateEls.forEach(birtdate => {
            var birtdateMonth = birtdate.querySelector('[data-component="birthdate-month"]')
            var birtdateYears = birtdate.querySelector('[data-component="birthdate-year"]')
            var birtdateDayEl = birtdate.querySelector('[data-component="birthdate-day"]');
            var monthIndex = 0;
            var year = 2023;

            birtdateMonth.addEventListener('change', e => {
                monthIndex = e.target.value;
                addOption(birtdateDayEl, monthIndex, year)
            })
            
            birtdateYears.addEventListener('change', e => {
                year = e.target.value;
                addOption(birtdateDayEl, monthIndex, year)
            })
        })
